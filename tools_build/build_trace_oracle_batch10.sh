...
# --- player.c:保留真實 get_player_data(回 data_C960 + player*512)。 ---
# b12 起,op_5D/5E/5F/60/61/62 都讀寫真實 data_C960;op_61 走 get_player_data,
# 故不可再 stub 成假緩衝。op_7D 的名字迴圈在 data_C960 全零時讀到第一個高位=0 的 byte
# 即收斂(更忠實),無需假緩衝。此處不改 player.c(維持 opendw 原樣)。
_=open(pl).read()  # no-op:保留原始 get_player_data
...
if 'static void sub_2AEE' not in s:
    shim=('// build shim\nstatic void sub_2AEE(){}\n'
      ...
      # b12:op_5F/op_60 在 disasm targets[] 標名 op_5F/op_60,實際 body 為
      #     op_or_char_data(0x4372)/op_and_char_data(0x438B)。alias 到真實 body
      #     才能逐指令對拍角色 bit 屬性 opcode(原本 batch10 stub 為空 → 不消耗 operand)。
      '#define op_5F op_or_char_data\n#define op_60 op_and_char_data\n'
      'static void op_or_char_data(); static void op_and_char_data();\n'
      'static void op_56(void); static void op_59(void); static void op_5C(void); static void op_72(void);\n'
      'static void op_43(void){} static void op_63(void){}\n')
...
// b12:測試前 seed game_state / char_data(data_C960)。供角色資料 opcode(5D/5E/5F/60/61/62)
//   對拍:remake 與 oracle 載入「同一份」game_state + char_data,再跑同一 bytecode。
//   注意:這些 setter 須在 dw_trace_run 之前呼叫;dw_trace_run 內的 memset(game_state) 會清掉,
//   故改為「dw_trace_run 不再清 game_state」,改由呼叫端決定(見下方 keep_gs 旗標)。
static unsigned char dw_seed_gs[256]; static int dw_have_seed_gs=0;
void dw_trace_seed_gs(int idx, int val){ if(idx>=0&&idx<256){dw_seed_gs[idx]=(unsigned char)val; dw_have_seed_gs=1;} }
extern unsigned char *get_player_data_base(void);
void dw_trace_seed_char(int addr, int val){ unsigned char*c=get_player_data_base(); if(addr>=0&&addr<0xE00) c[addr]=(unsigned char)val; }
void dw_trace_reset_seed(void){ dw_have_seed_gs=0; memset(dw_seed_gs,0,sizeof(dw_seed_gs)); unsigned char*c=get_player_data_base(); memset(c,0,0xE00); }
int dw_trace_get_char(int addr){ unsigned char*c=get_player_data_base(); return (addr>=0&&addr<0xE00)?c[addr]:0; }
void dw_trace_run(const unsigned char *code, int len){
  ...
  if(dw_have_seed_gs) memcpy(&game_state,dw_seed_gs,256); // b12:套用 seed(覆蓋 memset 的清零)
