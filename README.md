...
- ⚠️ **背景音樂：引擎就緒、待渲染素材**。**音效已實作**（見上「已落地」）。背景音樂的**引擎端循環播放系統也已就緒**（`sound.cpp` music 頻道 + 依遊戲狀態切 title/game/combat/end 四曲）；缺的是把 Amiga `.tune`（"Music by MANIACS of NOISE"，68k 機械碼播放器 + 內嵌曲目）渲染成 WAV——標準作法是 **UADE**（不需 Kickstart ROM），但開發沙箱網路受限無法在此抓取 UADE，故渲染步驟留給有網路的環境（見 [`bundle/audio/music/README.md`](opendw_remake/assets/bundle/audio/music/README.md) + `tools_build/render_music.sh`）。**WAV 放進 `bundle/audio/music/` 即自動循環播放**。（DOS 版原本就無背景音樂——其 `0x5C3B` 為 PC speaker 音效碼，非音樂。）
