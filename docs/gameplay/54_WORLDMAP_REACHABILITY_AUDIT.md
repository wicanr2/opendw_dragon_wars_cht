...
**結論(2026-06-20 二次深挖,定位精確化)**:菲巴斯入口**並非未解碼,而是「入口 tile 存在但未放置」**——

5. **權威反查確認**:area 0 script_table 中 **tile value 0x07 = 菲巴斯地點 tile**,其 script(@0x1297)
   `worldmap_dest=6`(有效的 →area 6 入口);且全檔僅此一處被 script_table 引用(0x11ce → 0x1297)。
6. **但 tile 0x07 未畫在 grid 上**:逐格掃 area 0(32×47)tile value,城市 tile 範圍 0x03–0x1F 中
   **唯獨 0x07(菲巴斯)缺席**,其餘 27 座城市全部放置。→ 菲巴斯的「門」定義完整,只是那格沒被畫上世界圖。
