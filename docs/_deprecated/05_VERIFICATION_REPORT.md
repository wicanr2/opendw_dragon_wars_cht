...
> ⚠️ **作廢/結論錯誤**:本報告宣稱「3,926 條已完整萃取、17/17 section 全部 ✅」,
> 但該萃取是「對每個 byte offset 暴力解 5-bit 壓縮」的產物,實為**雜訊**(同句滑動重複 + 亂碼),
> section 0x08–0x16 的「上百條 text」是假象。乾淨文字請見 `ALL_TEXT_FROM_SCRIPTS.txt`,
> 根因與正確方法請見 **`07_REVISED_PLAN.md`**。本檔以下統計**不可信**,僅供記錄。
