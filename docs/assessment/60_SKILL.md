...
> ⚠️ **註(2026-06-10)**:下方「3,926 個文字串/覆蓋率 2.5%」是早期暴力萃取的雜訊統計,**已修正**。
> 真實乾淨文字在 `ALL_TEXT_FROM_SCRIPTS.txt`,萃取方法(op_77/78/7b 後內嵌 5-bit 字串 + res 解碼)與最新進度
> 請見 `07_REVISED_PLAN.md`、`26_MONSTERS_AND_SPRITES.md`。反組譯還原(52 函式/143 opcode)仍可信。

**目前進度**（2026-06-09,部分數據已過時,見上方註記）：
- 52 個 unnamed 函式已更名
- 143 個 opcodes 已命名
- ~~3,926 個文字串從 DATA1 提取~~（為暴力萃取雜訊,見 07）
- 100+ 翻譯條目完成（覆蓋率非以 3,926 為分母計）
- 反組譯分析完成
- 中文化方案設計完成
- **實作尚未開始**（SDL2、CJK 渲染）
...
#### 翻譯檔案
- `docs/translation/10_TRANSLATION.md`：已知的中英文對照表（100+ 條目，2.5% 覆蓋率）
...
| `docs/reverse-engineering/01_PLAN.md` | 中文化實作計畫 |
| `docs/reverse-engineering/02_ANALYSIS.md` | 反組譯分析 |
| `docs/translation/10_TRANSLATION.md` | 翻譯對照表 |
...
| `docs/reverse-engineering/dragon.asm` | 反組譯參考 |
