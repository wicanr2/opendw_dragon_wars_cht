...
> ⚠️ **部分結論已過時/錯誤(2026-06-10 修正)**:本報告把「3,926 個文字串(優質成果)」「16 個 Section 完整掃描」
> 列為已完成成果並當作翻譯覆蓋率分母,**這是錯誤的** —— 該 3,926 條是「逐 byte 暴力解 5-bit 壓縮」的雜訊
> (同句滑動重複 + 亂碼),真實乾淨文字在 `ALL_TEXT_FROM_SCRIPTS.txt`。
> 因此本報告中所有以 3,926 為基準的「覆蓋率 2.5%」「3,870 條未翻譯」等推論均不成立。
> 根因與正確方法請見 **`07_REVISED_PLAN.md`**;怪物名正解見 `26_MONSTERS_AND_SPRITES.md`。
> 本報告其餘對「文件結構、反組譯還原、技術債、字型尺寸矛盾」的觀察仍有參考價值。
