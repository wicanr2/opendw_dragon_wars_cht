...
//   • 傷害/治療以 dice_count d dice_sides 表(docs/gameplay/58_MAGIC_REFERENCE.md 骰式);變動消耗(var.)法術以
...
// 全域法術表(數值權威 = docs/gameplay/58_MAGIC_REFERENCE.md)。每條:
...
const std::vector<SpellDef> kSpells = {
    // ── A. 初級法術 Low Magic(docs/gameplay/58_MAGIC_REFERENCE.md)──
    ...
    // ── B. 高級法術 High Magic(docs/gameplay/58_MAGIC_REFERENCE.md)──
    ...
    // ── D. 德魯伊法術 Druid Magic(docs/gameplay/58_MAGIC_REFERENCE.md)──
    ...
    // ── C. 太陽法術 Sun Magic(docs/gameplay/58_MAGIC_REFERENCE.md)──
    ...
    // ── E. 其他法術 Miscellaneous Magic(docs/gameplay/58_MAGIC_REFERENCE.md)──
...
ZapHit zap_attack_roll(int caster_int, int magic_ranks, const Combatant& target,
                       CombatRng& rng) {
  // docs/gameplay/58_MAGIC_REFERENCE.md:1d16+2 roll-under。命中 ⟺ roll ≤ 12 + ranks + INT − DV。
...
// 內部:把已擲出的傷害經 Zap 攻擊判定套到 target(命中全傷、miss 半傷,docs/gameplay/58_MAGIC_REFERENCE.md)。
static void apply_zap_damage(const SpellDef& sp, int caster_int, int magic_ranks,
  ...
  r.zap_roll = h.roll;
  r.zap_need = h.need;
  int dmg = h.hit ? raw_dmg : raw_dmg / 2;  // docs/gameplay/58_MAGIC_REFERENCE.md:miss 仍吃半傷(向下取整)
  r.amount = dmg;
  target.hp -= dmg;
  if (target.hp <= 0) {
    target.hp = 0;
...
CastResult cast_spell(std::uint8_t spell_id, int caster_power, int caster_str,
  ...
  switch (sp->effect) {
    case E::Damage: {
      // 固定骰式傷害(docs/gameplay/58_MAGIC_REFERENCE.md NdM,Kill Ray 為平坦區間),走 Zap 攻擊判定(miss 半傷)。
      int raw = roll_damage_dice(*sp, rng);
      apply_zap_damage(*sp, caster_int, magic_ranks, raw, target, rng, r);
      break;
    }
    case E::PowerScaled: {
      // docs/gameplay/58_MAGIC_REFERENCE.md「NdM hp/pt」:每投入 1 點 Power 擲一份 dice_count d dice_sides。
    ...
    case E::BuffAv:
      ...
      // Charm(0x02)docs/gameplay/58_MAGIC_REFERENCE.md:除 +1 AV 外另治 1d4 hp。以 dice_count/dice_sides 表治療骰。
      if (sp->dice_count > 0 && sp->dice_sides > 0) {
    ...
    case E::BuffDex:
      ...
      break;
    case E::DebuffAvDv: {
      // 蟲災(Insect Plague,docs/gameplay/58_MAGIC_REFERENCE.md):整場 −2 AV、−2 DV;Debuff 類走 Zap 攻擊判定。
      //   命中 → 套滿 −2/−2;miss → 半效(−1/−1,向下取整;docs/gameplay/58_MAGIC_REFERENCE.md「miss 吃半傷」推及 debuff)。
      r.is_zap = true;
      ZapHit h = zap_attack_roll(caster_int, magic_ranks, target, rng);
...
Combatant make_summon(SummonKind kind, int caster_str) {
  // 召喚物屬性 = **docs/gameplay/58_MAGIC_REFERENCE.md 召喚表精確值**(HP / Dex / Armor / 武器傷害骰)。
  //   AV/DV = Dex/4(docs/gameplay/58_MAGIC_REFERENCE.md「AV/DV 由 Dex 推,同怪物 DEX/4 慣例」);AC = Armor;
  //   武器骰直接照表(Air 1d10、Earth 1d20、Fire 2d20…)。真值層級 = remake 設計
  //   (grounded docs/gameplay/58_MAGIC_REFERENCE.md 攻略,非 bytecode oracle)。確定性(不耗 RNG)。
  ...
  switch (kind) {
    ...
    case SummonKind::Salamander:
      // docs/gameplay/58_MAGIC_REFERENCE.md:很危險的咒語(可能反噬施法者);反噬機率/數值攻略未量化 → 不臆造,
      //   只給攻略表的召喚物屬性。
  ...
  u.name = st.name;
  u.hp = u.max_hp = st.hp;
  u.av = st.dex / 4;    // docs/gameplay/58_MAGIC_REFERENCE.md:AV/DV = Dex/4(同怪物慣例)
  u.dv = st.dex / 4;
  u.ac = st.armor;
  u.dmg_dice = st.dice;
