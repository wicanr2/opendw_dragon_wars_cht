# CONTEXT.md — 火龍之戰中文化術語表 (Ubiquitous Language)

> 本檔為 Dragon Wars 中文化的**標準術語對照**。命名變數、寫翻譯、寫文件一律以此為準。
> 譯名優先序:**官方臺灣中文手冊(珍066-火龍之戰)** > 約定俗成 > 直譯。
> 格式:`English — 中文. _Avoid_: 禁用同義詞`

---

## 地名 / Places

- **Purgatory — 波卡城**(地名)/ **罪惡之城**(描述). _官方手冊雙譯:地名用音譯「波卡城」,敘述語境用「罪惡之城」。經掃描圖核對_. _Avoid_: 煉獄、滌罪所
- **Morgue — 陳屍所**. _官方手冊譯名(掃描圖核對)_
- **Silver Wheel — 銀輪**. _官方手冊譯名_
- **Magan / Magan Underworld — 瑪根 / 瑪根地底世界**. _官方手冊譯名_
- **Mountain of Salvation — 救贖之山**
- **Well of Souls — 靈魂之泉**. _瑪根地底世界中讓亡魂復生之處_
- **Court of Miracles — 奇蹟宮廷**. _Clopin Trouillefou 的地盤_
- **the Pit — 深淵 / 龍坑**. _Namtar 來自之處;餵龍的坑_
- **the harbor — 港口**
- **Oceana — 歐西納**. _官方手冊(太陽系名)_
- **Dilmum — 迪瑪**. _官方手冊(行星名)_
- **Nisir — 尼塞**. _官方手冊(山脈,瑪根地底世界所在)_
- **Kingshome — 京雄城**. _官方手冊地圖_
- **Byzanople — 拜占儂**. _官方手冊地圖_
- **Mud Toad / Yellow Mud Toad — 黃泥蟾蜍城**. _官方手冊地圖_
- Tars / Tarsus / Lansk / Zaton — _保留原文待確認_

## 人物 / Characters

- **Namtar — 納達**. _官方手冊譯名_. The Beast From The Pit — **深淵之獸**. _Avoid_: 納姆塔(非官方)
- **Humbaba — 胡姆巴巴**. _遊戲中段大魔王_
- **Irkalla — 伊爾卡拉**. _瑪根地底世界的黑暗之后 / 冥界女王(Queen of the Underworld);可向其祈禱復活亡魂。手冊段落 137 中被銀鍊綁住者_
- **Nergal — 奈羅**. _Irkalla 之**夫**(consort),獨立冥神角色。英文手冊確認「Irkalla… and her consort Nergal」_. **⚠ 勿與 Namtar(納達,最終魔王)混淆** —— 軟體世界攻略 OCR 曾將 Nergal 誤判為納達
- **Clopin Trouillefou — 克洛潘·特魯伊弗**. _奇蹟宮廷之首_
- Enkidu / Utnapishtim — _保留原文待確認_

## 屬性 / Stats (角色狀態畫面)

- **Health (Health) — 生命**. _Avoid_: 血量、HP
- **Stun — 暈眩值**. _Avoid_: 昏迷
- **Power / Spell Power — 法力**. _Avoid_: 魔力、MP、魔法值
- **Str (Strength) — 力量**
- **Dex (Dexterity) — 敏捷**
- **Int (Intelligence) — 智力**
- **Spr (Spirit) — 精神**
- **Attack — 攻擊**；**Defense — 防禦**；**Level — 等級**
- **AC (Armor Class) — 防禦等級**. _Avoid_: 護甲級別
- **Exp (Experience) — 經驗**；**Gold — 金幣**；**Advancement points — 成長點數**

## 法術系統 / Magic

- **Low Magic — 初級魔法**；**High Magic — 高級魔法**. _官方手冊用「初級/高級」_. _Avoid_: 低階/高階
- **Druid (Magic) — 德魯伊魔法**；**Sun (Magic) — 太陽魔法**
- **Miscellaneous / Misc Magic — 雜項魔法**
- **Whirl Wind — 龍捲風**. _官方手冊譯名_
- **Summon Salamander — 召喚火蜥蜴**；**Beast Call — 呼叫野獸**. _官方手冊譯名_
- Major Heal — 高階治療；Mighty Blow — 強力一擊；Mystic Might — 神祕之力
- 法術詞條格式:`Target 目標 / Range 範圍 / Effect 效果 / Power 消耗 / Time 持續`

## 戰鬥 / Combat

- **Encounter — 遭遇戰**. _Avoid_: 遇敵
- **Fight — 戰鬥**；**Quickly fight — 快速戰鬥**；**Run — 逃跑**；**Advance — 前進**
- **Attack 攻擊 / Dodge enemies 閃避 / Block attack 格擋 / Cast spell 施法 / Use item 使用物品**
- **damage — 傷害**；**point of damage — 點傷害**；**stun — 暈眩(動詞)**
- **deequip/unequip — 卸下**；**equip — 裝備**；**reload/Load weapon — 裝填**
- **gladiator — 鬥士**；**arena — 競技場**

## 物品 / Items

- **item — 物品**；**weapon — 武器**；**Gold — 金幣**；**loot — 戰利品**
- **Pool gold 集中金幣 / Share gold 平分金幣 / Trade gold 交易金幣**
- **Discard — 丟棄**；**Trade — 交易**；**Transfer — 轉移**

## 怪物 / Monsters (res31,官方譯名待手冊核對)

| English | 中文(暫定) |
|---------|-----------|
| Wolf / Wild Dog / Big Dog / Wild hound | 野狼 / 野狗 / 大狗 / 野獵犬 |
| Spider / Rock Spider / Giant Spider | 蜘蛛 / 岩蜘蛛 / 巨蜘蛛 |
| Guard / King's Guard / Soldier | 守衛 / 國王守衛 / 士兵 |
| Robber / Bandit / Cannibal | 強盜 / 盜匪 / 食人者 |
| Loon / Fanatic / Yonderboy / Born Loser | 瘋子 / 狂熱者 / 遠方少年 / 天生輸家 |
| Pikeman / Gladiator / Jail Keeper / Drunk | 長矛兵 / 鬥士 / 獄卒 / 醉漢 |
| Innocent Man / Unjustly Accused | 無辜者 / 蒙冤者 |
| Humbaba | 胡姆巴巴 |
| Yellow Mud Toad / Wood Spirit | 黃泥蟾蜍 / 木靈 |

## UI / 系統訊息

- **party — 隊伍**；**character — 角色**；**Create/Delete/Rename/View — 建立/刪除/重新命名/檢視**
- **Loading... — 載入中…**；**Saving game... — 儲存遊戲中…**；**The game is paused — 遊戲已暫停**
- **paragraph (Read paragraph N) — 段落(請閱讀第 N 段)**. _防拷段落,內容在手冊_

## 地圖導航 / Map navigation(換場機制,逆向自 opendw)

- **area — 區域**. 一張地圖關卡(共 40 關)；存於 `game_state[2]`，資源 index = `area + 0x46`. _Avoid_: level(避免與 .lvl 檔混用)、world
- **entry x/y/facing — 入口座標/朝向**. 換場後玩家落點;存於 `gs[0]`=X、`gs[1]`=Y、`gs[3]`=朝向(0=N 1=E 2=S 3=W)
- **area switch — 換場(換 area)**. 事件腳本寫 `gs[2]`=新 area → poll 偵測 `gs[2]` 變化 → 重載 `maps/<area>.lvl`. opendw 走 `load_level_resources`(每幀比對 `gs[2]` vs `gs[0x57]`)
- **relocation / 傳送**. 同一 area 內事件改 `gs[0]/gs[1]`(樓梯/陷阱),不重載地圖. _Avoid_: 把同區傳送說成「換場」
- **wrap world — 環繞世界**. level header flag(`gs[0x23]`)bit1 設定;opendw 對其邊界環繞與「換入此 area」皆 `exit(1)` 未實作. remake 規範:**換入 wrap-world 目標 → 跳過 + log**(area 0 Dilmun、18、19、22、27 等)
- **sync_relocation**. remake main 中對拍 `load_level_resources` 的 poll 函式:跑完事件腳本後比對 `gs[2]`/`gs[0..1,3]` → 換 area / 同區傳送 / wrap 跳過

---

## Flagged ambiguities(待釐清)

- `Power` 一詞雙義:角色屬性「法力」 vs 法術消耗「消耗」。畫面依上下文區分。
- 多數地名/人名(Oceana/Tars/Nisir/Lansk/Enkidu)手冊 OCR 不清,**暫保留原文**,待人工核對掃描圖再定譯名。
- 怪物中文名為暫定,需與手冊掃描圖最終核對(見 `docs/26_MONSTERS_AND_SPRITES.md`)。
- **句子碎片問題**:遊戲以「片段 + 變數」組句(英語語序)。中文翻譯時語序需重排,部分組合句需在 #2 階段調整 opcode 或片段切法,不能逐片直譯。詳見 `docs/15_TRANSLATION_DRAFT.md`。
